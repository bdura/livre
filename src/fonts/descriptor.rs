...
use crate::{extraction::{extract, Name}, Extract, Rectangle};
...
mod tests {
    ...
    fn font_flags(#[case] input: &[u8], #[case] expected: FontFlags) {
        let flags = extract(&mut input.as_ref()).unwrap();
    ...
    #[case(
        ...
        "},
        FontDescriptor { 
            font_name: Name::from("AGaramond-Semibold"),
            font_family: None,
            font_stretch: None,
            font_weight: None,
    ...
    fn font_descriptor(#[case] input: &[u8], #[case] expected: FontDescriptor) {
        let fd = extract(&mut input.as_ref()).unwrap();
